...
function ensureProfileShape(p) {
  const tags = Array.isArray(p && p.tags) ? p.tags.map(normalizeTag) : [];
  return {
    id: String(p && p.id ? p.id : makeId("profile")),
    name:
...
function profilePrompt(profile, fallbackTopic) {
  const p = profile || buildDefaultProfile();
...
function heuristicIrrelevant(tile, profile, fallbackTopic) {
  const text = [tile.title, tile.channel, tile.description]
    .join(" ")
    .toLowerCase();
  const tags = uniqStrings([
    ...((profile && profile.tags) || []),
    ...topicWords(fallbackTopic),
  ])
    .map(normalizeTag)
    .filter(Boolean);
...
function applyOverlayFn({ blockedVideoIds, blockedItemKeys }) {
  ...
  const maybeBlockHost = (host) => {
    ...
    for (const a of links) {
      ...
      let full = "";
      try {
        full = new URL(raw, location.origin).toString();
      } catch (_) {
        full = raw;
      }
...
function scrapeFn({ mode, lookahead }) {
  ...
  const firstHref = (el) => {
    const cand = el.querySelector(
      "a#thumbnail[href], a#video-title-link[href], a#video-title[href]," +
        ...
        "a[href*='/watch?v='], a[href*='/shorts/'], a[href*='/playlist?list=']," +
        "a[href*='/browse/'], a[href]",
    );
    if (!cand) return "";
  ...
  const firstThumb = (el, vid) => {
    ...
    if (srcset) {
      ...
      const ytEntry = [...entries]
        .reverse()
        .find((u) => u.includes("ytimg.com"));
  ...
  for (const sel of SELECTORS) {
    ...
    for (const el of nodes) {
      ...
      out.push({
        ...
        thumbnail_url: firstThumb(el, vid),
        url,
        tile_type: sel,
        scraped_at: ts,
        page_url: location.href,
  ...
  // ── Wide scan: catch anything the structured pass missed ──────────────────
  ...
  for (const a of links) {
    ...
    const vid = videoIdFromUrl(url);
    const text = (a.getAttribute("title") || a.textContent || "")
      .replace(/\s+/g, " ")
      .trim();

    // Skip duration-badge anchors like "18:11"
    ...
    const channel = (() => {
      ...
      for (const sel of CHANNEL_SEL_WS) {
    ...
    out.push({
      ...
      channel,
      description: "",
      thumbnail_url: firstThumb(card, vid),
      url,
  ...
  try {
    console.log("[LockIn] scrape counts:", counts);
  } catch (_) {}

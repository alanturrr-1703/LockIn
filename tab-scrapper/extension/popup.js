...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const activeProfile =
    profiles.find((p) => p.id === stored.activeProfileId) ||
  ...
  if (activeProfile) {
    ...
    tags.textContent =
      activeProfile.tags && activeProfile.tags.length
        ? activeProfile.tags.join(", ")
        : "No tags yet.";
    ...
    if (!stored.lastResult) {
      setStatus(`${activeProfile.name}. Ready.`);
    }
  } else {
    const name = document.createElement("strong");
  ...
  $("topic").addEventListener("change", () =>
    chrome.storage.local.set({ topic: $("topic").value }),
  );
